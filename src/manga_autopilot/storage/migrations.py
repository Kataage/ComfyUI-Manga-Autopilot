...
def _cleanup_owned_staged_database(path: Path) -> None:
    ...
    for candidate in (
        ...
        try:
            ...
            # Cleanup must never mask the primary migration/publish failure.
            pass
...
class MigrationRunner:
    ...
    def migrate(self, database_path: str | Path) -> MigrationResult:
        ...
        if not path.exists():
            return self._migrate_new_database_safely(path)
        return self._migrate_in_place(path)

    def _migrate_new_database_safely(self, path: Path) -> MigrationResult:
        """Build a fresh DB under an owned name, then publish without overwrite."""
        staged = path.with_name(
            f".{path.name}.init-{uuid.uuid4().hex}"
        )
        try:
            result = self._migrate_in_place(staged)

            with write_connection(staged) as connection:
                checkpoint = connection.execute(
                    "PRAGMA wal_checkpoint(TRUNCATE)"
                ).fetchone()
                if checkpoint is not None and int(checkpoint[0]) != 0:
                    raise MigrationIntegrityError(
                        f"WAL checkpoint was busy for staged database {staged}: "
                        f"{tuple(checkpoint)}"
                    )
            ...
            try:
                ...
                return self._migrate_in_place(_migration_database_path(path))
            except OSError as exc:
                raise MigrationError(
                    f"failed to publish fresh database without overwrite: {path}"
                ) from exc
